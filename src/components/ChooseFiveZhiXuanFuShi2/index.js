...
class ChooseFiveZhiXuanFuShi2 extends React.PureComponent {
  ...
  remove(val) {
    ...
    const numbersTwo = [...statusTwo] && [...statusTwo].sort() && [...statusTwo].sort(function(a, b) {return a-b }).join('-');
  ...
  changeState(value) {
    if(this.state.status.indexOf(value) !== -1) {
      ...
      const numbersOne = status && status.sort(function(a, b) {return a-b }).join('-');
      const numbersTwo = [...this.state.statusTwo].sort(function(a, b) {return a-b }).join('-');
      changeSelectLotteryNumbers(numbersOne + ',' + numbersTwo);
    }
  }
  removeTwo(val) {
  ...
  changeStateTwo(value) {
    if(this.state.statusTwo.indexOf(value) !== -1) {
      ...
      // 添加
      const statusTwo = [...this.state.statusTwo, value];
      this.setState({ statusTwo, type2: '' });
      const numbersOne = [...this.state.status].sort(function(a, b) {return a-b }).join('-');
...
export default ChooseFiveZhiXuanFuShi2;
